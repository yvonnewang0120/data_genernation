...
def SD2_instance_generator(config):
    ...
    return job_length, op_pt, op_per_mch, setup_time_matrix
...
def generate_setup_table(op_pt, A, B, seed=24, as_int=True):
    ...
    assert total_rows % M == 0, "op_pt_flat 的行数必须能被列数 M 整除，以恢复 N"
    N = total_rows // M
    total_ops = M * N
...
def pack_data_from_config(data_source, test_data):
    ...
    for data_name in test_data:
        data_path = f'./data/data_train_vali/{data_source}/{data_name}'     #！！！！！！
...
def generate_data_to_files(seed, directory, config):
    ...
    if (not os.path.exists(path)) or config.cover_data_flag:
        ...
        for idx in range(batch_size):
            if source == 'SD2':
                job_length, op_pt, op_per_mch, _ = SD2_instance_generator(config=config)
...
class CaseGenerator:
    ...
    def get_case(self, idx=0):
        ...
        nonzero_pt = op_pt[op_pt > 0]
        if len(nonzero_pt) == 0:
            tmp_max = 1
            tmp_min = 0
        else:
            tmp_max = np.max(nonzero_pt)
            tmp_min = np.min(nonzero_pt)
        max_time = tmp_max
        min_time = tmp_min

        max_setup = max_time * 0.2
        min_setup = min_time * 0.05

        n_op = np.sum(job_length)  # total operations => N

        setup_time_matrix = np.zeros((n_op, n_op), dtype=int)

        # base fill
        for i in range(n_op):
            for j in range(n_op):
                if i != j:
                    val = random.randint(int(min_setup) + 1, max(int(max_setup), int(min_setup) + 1))
                    setup_time_matrix[i, j] = val

        # pick 5%~10% columns => big val
        for i in range(n_op):
            low_count = max(1, int(0.05 * n_op))
            high_count = max(low_count, int(0.1 * n_op))
            big_count = random.randint(low_count, high_count)
            cols = list(range(n_op))
            if i in cols:
                cols.remove(i)
            if len(cols) == 0:
                continue
            chosen_cols = random.sample(cols, min(big_count, len(cols)))
            for c in chosen_cols:
                big_val = random.randint(int(0.8 * max_time),
                                         max(int(1.1 * max_time), int(0.8 * max_time) + 1))
                setup_time_matrix[i, c] = big_val

        # dummy row => shape => [n_op+1, n_op]
        # for each of n_op => random from [min_setup, max_setup]
        dummy_row = [random.randint(int(min_setup) + 1,
                                    max(int(max_setup), int(min_setup) + 1))
                     for _ in range(n_op)]
        setup_time_matrix = np.insert(setup_time_matrix, 0, dummy_row, axis=0)

        return job_length, op_pt, self.num_options / self.num_opes, setup_time_matrix
